...
class AppFolioIntegration(Integration):
    ...
    async def fetch_email_messages(
        ...
        omit_bodies: bool = True,
    ):
        """Fetch email records filtered by one or more AppFolio short UIDs."""
        if not short_uids:
            raise ValueError("At least one short UID is required")

        params = [
            ("items_per_page", str(items_per_page)),
            ("omit_bodies", str(omit_bodies).lower()),
            ("page", str(page)),
        ]
        params.extend(("short_uids[]", str(uid)) for uid in short_uids)
        headers = {
        ...
        response = await self._make_request(
            "GET", f"{self.url}/emails", params=params, headers=headers
        )

        try:
            payload = json.loads(response)
            if isinstance(payload, dict) and "body_row_data" in payload:
                return self._parse_email_table(payload, page)
            return payload
        ...
        # Some AppFolio pages place their API result in an application/json script.
        embedded_json = []
        for script in soup.select('script[type="application/json"]'):
            try:
                embedded_json.append(json.loads(script.get_text(strip=True)))
            except (json.JSONDecodeError, TypeError):
                continue
        if embedded_json:
            return {"emails": embedded_json, "source": "embedded_json"}

        # Fallback for server-rendered email tables/cards. Keep raw text so a new
        # AppFolio layout does not silently discard data we have not mapped yet.
        records = []
        selectors = (
            "tr[data-email-id], [data-email-id], .email-row, .email-item, "
            ".email-card, table tbody tr"
        )
        for node in soup.select(selectors):
            ...
            records.append({
                "id": node.get("data-email-id") or node.get("data-id"),
                "subject": field(".subject", "[data-field='subject']"),
                "from": field(".from", ".sender", "[data-field='from']"),
                "to": field(".to", ".recipient", "[data-field='to']"),
                "date": field("time", ".date", ".sent-at", "[data-field='date']"),
                "body": field(".body", ".message-body", "[data-field='body']"),
                "raw_text": text,
            })

        return {"emails": records, "source": "html", "page": page}

    @staticmethod
    def _parse_email_table(payload: dict, requested_page: int) -> dict:
        """Normalize AppFolio's JSON response containing HTML table cells."""
        heading_soup = BeautifulSoup(payload.get("thead_row", ""), "html.parser")
        headings = [element.get_text(" ", strip=True) for element in heading_soup.select("th")]
        emails = []

        for row in payload.get("body_row_data", []):
            cells = row.get("data", [])
            values = []
            has_attachment = False
            for cell in cells:
                raw_value = html.unescape(str(cell.get("value", "")))
                cell_soup = BeautifulSoup(raw_value, "html.parser")
                if cell_soup.select_one(".email-block--attachment-icon"):
                    has_attachment = True
                values.append(cell_soup.get_text(" ", strip=True))

            fields = dict(zip(headings, values))
            attributes = {
                item.get("key"): item.get("value")
                for item in row.get("row_data_attributes", [])
                if item.get("key")
            }
            href = attributes.get("href")
            id_match = re.search(r"/emails/(\d+)", href or "")
            emails.append({
        ...
        pagination_html = payload.get("pagination", "")
        summary_soup = BeautifulSoup(pagination_html, "html.parser")
        summary_node = summary_soup.select_one(".js-pagination-summary")
        summary = summary_node.get_text(" ", strip=True) if summary_node else ""
        summary_match = re.search(r"Displaying:\s*(\d+)-(\d+)\s+of\s+(\d+)", summary)
        last_page = requested_page
        for link in summary_soup.select("a.js-pagination-link[href]"):
            query = urllib.parse.parse_qs(urllib.parse.urlparse(link["href"]).query)
            try:
                last_page = max(last_page, int(query.get("page", [requested_page])[0]))
        ...
        return {
            ...
            "pagination": {
                "page": requested_page,
                "last_page": last_page,
                "total": int(summary_match.group(3)) if summary_match else len(emails),
                "displaying_from": int(summary_match.group(1)) if summary_match else None,
                "displaying_to": int(summary_match.group(2)) if summary_match else None,
            },
...
async def _run_cli(args):
    ...
    await integration.initialize(domain=args.domain, tokens=args.cookie)

    if args.command == "tenants":
        result = await integration.fetch_all_tenants(page=args.page)
    elif args.command == "properties":
        result = await integration.fetch_properties(page=args.page)
    elif args.command == "vacancies":
        result = await integration.fetch_vacancies()
    elif args.command == "moves":
        result = await integration.fetch_tenancy_move_data()
    elif args.command == "work-orders":
        result = await integration.fetch_work_orders(
            status=args.status, start_date=args.start_date, page_number=args.page
        )
    elif args.command == "emails":
        result = await integration.fetch_email_messages(
            short_uids=args.short_uid,
            page=args.page,
...
def _build_cli_parser():
    parser = argparse.ArgumentParser(description="Read data from an AppFolio account.")
    parser.add_argument(
        "--domain", default=os.getenv("APPFOLIO_DOMAIN"),
        help="AppFolio hostname, or set APPFOLIO_DOMAIN",
    )
    parser.add_argument(
        "--cookie", default=os.getenv("APPFOLIO_COOKIE"),
        help="Authenticated Cookie header, or set APPFOLIO_COOKIE",
    )
    commands = parser.add_subparsers(dest="command", required=True)
    tenants = commands.add_parser("tenants", help="Fetch tenants")
    tenants.add_argument("--page", type=int, default=1)
    properties = commands.add_parser("properties", help="Fetch properties")
    properties.add_argument("--page", type=int, default=1)
    commands.add_parser("vacancies", help="Fetch vacancies")
    commands.add_parser("moves", help="Fetch tenancy move data")
    work_orders = commands.add_parser("work-orders", help="Fetch work orders")
    work_orders.add_argument("--status", required=True)
    work_orders.add_argument("--start-date", required=True, help="YYYY-MM-DD")
    work_orders.add_argument("--page", type=int)
    emails = commands.add_parser("emails", help="Fetch emails by tenant short UID")
    emails.add_argument("--short-uid", action="append", required=True)
...
def main():
    ...
    if not args.domain:
        parser.error("--domain is required (or set APPFOLIO_DOMAIN)")
    if not args.cookie:
        parser.error("--cookie is required (or set APPFOLIO_COOKIE)")
